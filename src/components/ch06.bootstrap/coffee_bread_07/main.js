...
function App() {
    ...
    */
    ...
    const Ordering = (orderInformation) => {
        const column = orderInformation.column; // 정렬할 때 사용한 컬럼
    ...
    const ClickOrderBy = (column_name, order_by) => {
        ...
        Ordering(newOrderInfo);//정렬 합수를 호출합니다.
    }

    /* 필드 검색 기능 추가 */
    /*필드 검색시 내가 선택한 category의 영문 이름이 저장되는 state 입니다. */
